...
async function main() {
    ...
    if (process.argv[3] == "deploy") {
        ...
        exec(
          `cd chains/evm && forge install && forge build && forge create --legacy --rpc-url ${network.rpc} --private-key ${network.privateKey} src/XFlow.sol:XFlow && exit`,
          (err, out, errStr) => {
            if (err) {
              throw new Error(err);
            }
        ...
        const flow = new ethers.Contract(
            ...
            JSON.parse(
                fs
                    .readFileSync(
                        "./chains/evm/out/XFlow.sol/XFlow.json"
                    )
                    .toString()
            ).abi,
            signer
        );
        await flow.registerApplicationContracts(
        ...
        console.log(`${process.argv[2]} flow has ${await flow.getTKNCount()} tokens.`);
        ...
        console.log(`${process.argv[2]} flow has ${await flow.getTKNCount()} native TKN.`)
        if(deployment[process.argv[2]]['wrappedTestTokenAddress']){
            console.log(`${process.argv[2]} flow has ${await flow.getWrappedCount(deployment[process.argv[2]]['wrappedTestTokenAddress'])} wrapped TKN.`)
        }
        ...
        // Remember to allow Token Bridge to move tokens from flow account to it's own account
        console.log(`Approving ${process.argv[5]} (${bridgeAmt} in proper ERC20 format) Tokens to be bridged by Token Bridge`);
        await flow.approveTokenBridge(bridgeAmt, {
            gasLimit: 2000000,
        });
        await new Promise((r) => setTimeout(r, 5000)); //Time out to let block propogate
        ...
        const tx = await (await flow.bridgeToken(bridgeAmt, targetNetwork.wormholeChainId, targetRecepient)).wait();
        const emitterAddr = getEmitterAddressEth(network.tokenBridgeAddress);
